...
- Creator: `drl33+creator@creatorbridge.studio` / password generated by `npm run qa:create-accounts`
- Client: `drl33+client@creatorbridge.studio` / password generated by `npm run qa:create-accounts`

Do not commit QA passwords. Store current QA credentials only in local `.env` or the appropriate secret store.

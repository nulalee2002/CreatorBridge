-- Allow contact details in direct messages only after the two users have an
-- active paid booking together. Before that point, the database keeps the
-- existing anti-circumvention filter as the source of truth.
...
begin
  ...
  if v_contact_pattern then
    select exists (
      ...
      join public.creator_listings cl on cl.id::text = t.creator_id
      where (

...
- [ ] Run one live browser smoke pass across the core public and logged-in flows: homepage, find creators, creator profile, project board, support ticket, notification center, creator dashboard gate, client dashboard gate, admin support, admin operations search, admin finance, admin analytics, and creator agreement print. As part of this gate, run `npm run verify:workflow-smoke` from the normal Mac terminal once before launch; Codex could not run this remotely because the app sandbox could not resolve the Supabase host.

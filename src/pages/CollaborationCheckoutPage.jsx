...
function ACHForm({ summary }) {
  ...
  return <form onSubmit={submit} className="space-y-5"><div className="rounded-xl border border-gold-500/20 bg-gold-500/10 p-4 text-sm"><div className="flex justify-between"><span>Collaboration</span><strong>${(summary.baseAmountCents/100).toFixed(2)}</strong></div><div className="mt-2 flex justify-between"><span>ACH processing cost</span><strong>${(summary.processingCostCents/100).toFixed(2)}</strong></div><div className="mt-3 flex justify-between border-t border-gold-500/20 pt-3"><span>Total bank debit</span><strong>${(summary.primeChargeCents/100).toFixed(2)}</strong></div><p className="mt-3 text-xs text-charcoal-300">Buyer platform fee: $0. You cover the bank-processing cost. The collaborator pays their earned {summary.creatorFeePct}% CreatorBridge creator fee from the payout. Do not begin work until funded.</p></div><PaymentElement options={{ layout:'tabs' }}/>{error&&<p className="text-sm text-red-300">{error}</p>}<button disabled={!stripe||saving} className="btn-gold w-full justify-center py-3">{saving?'Confirming ACH…':'Fund collaboration by ACH'}</button></form>;
}

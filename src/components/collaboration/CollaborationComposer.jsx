    ...
    <div className="fixed inset-0 z-[230] flex justify-end bg-black/75 backdrop-blur-sm" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <form onSubmit={submit} className="h-full w-full max-w-2xl overflow-y-auto border-l border-gold-500/20 bg-charcoal-950 p-6 text-white sm:p-8">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          <label className="text-xs">Fixed collaborator payout<input type="number" min="250" step="1" value={amount} onChange={(e) => setAmount(e.target.value)} className="mt-2 w-full rounded-xl border border-white/10 bg-charcoal-900 p-3" placeholder="$250 minimum" required/></label>
        ...
        <div className="mt-5 rounded-xl border border-gold-500/20 bg-gold-500/10 p-4 text-xs leading-6 text-charcoal-200"><strong className="text-gold-400">Before you invite:</strong> $250 minimum · fixed collaborator payout · funded upfront by ACH before work begins · released after approval or the auto-release window · no buyer platform fee · disclosed processing cost · private team workspace. The hiring creator covers bank processing. The hired creator pays their normal CreatorBridge creator fee from the payout. The outside client will not see the subcontractor or the production-team workspace.</div>

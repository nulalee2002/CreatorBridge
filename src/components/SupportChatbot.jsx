...
PAYMENTS:
Clients pay 50 percent retainer upfront with no fees added. The remaining 50 percent plus the one-time 5 percent booking fee is attempted when the client approves delivery, or automatically after a five-day review window if the client does not respond. Funds release only after Stripe confirms payment.
...
const CREATOR_STEPS = [
  ...
  },
  {
    ...
    question: 'CreatorBridge includes two revisions with every project.',
    type: 'options',
    options: ['2 included revisions'],
  },
  {
    step: 6, field: 'notes',
...
export function SupportChatbot({ dark = true }) {
  ...
  async function handleSend() {
    ...
    try {
      ...
      setError(`Could not reach support. Try emailing ${SUPPORT_EMAIL}`);
    } finally {
      setLoading(false);
    }
  }

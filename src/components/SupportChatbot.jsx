...
PAYMENTS:
Clients pay 50 percent retainer upfront with no fees added. The remaining 50 percent plus the one-time 5 percent booking fee is due when the client approves delivery, or automatically after a 72-hour review window if the client does not respond. All payments processed through Stripe.

PROJECT AGREEMENTS:
When a client accepts a creator proposal, CreatorBridge prepares a production agreement from the accepted brief, proposal amount, and selected creator package. The client and creator must each review and sign the same agreement before the retainer can be paid. A saved signature makes repeat signing faster, but it never signs a new agreement automatically. Each signature records the signer name, timestamp, IP, device, and the hash of the exact terms signed. This is a first-party electronic signature flow and requires entertainment-attorney review before production reliance.

REBOOKING:
Past clients can rebook a creator from a completed project, the creator profile, project history, or Saved Creators. CreatorBridge copies the prior creator, package, and brief into an editable repeat-booking request. The client adds the new date and can adjust the scope and amount before sending. The creator must confirm, then a fresh agreement and two fresh signature consents are required. Rebooking never changes or reuses the old contract itself.
...
DISPUTES:
After delivery, the client has a 72-hour review window to approve, request a revision, or open a dispute, any of those actions pauses the automatic release. If the client takes no action within 72 hours, payment auto-releases to the creator and disputes can no longer be opened. Valid dispute reasons: work does not match the agreed brief, significantly fewer deliverables than agreed, technical quality makes work unusable, creator abandoned the project. Not valid: client changed their mind after delivery, wanting more than the 2 included revisions, minor style preferences. For urgent disputes email drl33@creatorbridge.studio with URGENT in the subject line.
